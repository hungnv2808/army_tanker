...
/// <summary>
/// Combine mesh of all child objects have same material and each child object has only one material
/// </summary>
public class MeshCombineWizard : ScriptableWizard
{
	...
	void OnWizardCreate()
	{
		...
		foreach (var entry in materialToMeshFilterList) {
			...
			AssetDatabase.CreateAsset(combinedMesh, "Assets/Resources/Prefabs/Combine Meshes/CombinedMeshes_" + materialName + ".asset");
		...
		// Create prefab
		// Object prefab = PrefabUtility.CreateEmptyPrefab("Assets/Resources/Prefabs/Combine Meshes/" + resultGO.name + ".prefab");
		// PrefabUtility.ReplacePrefab(resultGO, prefab, ReplacePrefabOptions.ConnectToPrefab);
		PrefabUtility.SaveAsPrefabAssetAndConnect(resultGO, "Assets/Resources/Prefabs/Combine Meshes/" + resultGO.name + ".prefab", InteractionMode.AutomatedAction);
